...
class MemoryMapperLogic(logicRunnerPlugin.logicRunner):
    ...
    def doMagic(self, config):
        try:
            ...
            raise Exception(
                f"MemoryMapper is missing required attribute, more info : {str(e)}"
            ) from e
    ...
    def mapKernelStacks(self):
        for kernel in self.kernels:
            ...
            if not lowAddress or not highAddress:
                raise ValueError(
                    f"Kernel for core {kernel.core.name} with stack size :{kernel.stackSize} bytes cannot be allocated in the {stackMemory}\
        ...
        for osPartition in self.osPartitions:
            if osPartition.size:
                lowAddress, highAddress = self.allocateMemory(
                    osPartition.memory, osPartition.size
                )
                if not lowAddress or not highAddress:
                    raise ValueError(
                        f"OsPartition {osPartition.name} with size :{osPartition.size} bytes cannot be allocated in the {osPartition.memory}\
    ...
    def mapThreadStacks(self):
        for thread in self.threads:
            ...
            if not lowAddress or not highAddress:
                raise ValueError(
                    f"Thread {thread.name} stack with size :{thread.stackSize} bytes cannot be allocated in the {stackMemory}\
									cause it has only {self.returnFreeBytes(stackMemory)} free bytes"
                )
            else:
    ...
    def mapProgramData(self):
        for program in self.programs:
            if program.size:
                lowAddress, highAddress = self.allocateMemory(
                    program.memory, program.size
                )
                if not lowAddress or not highAddress:
                    raise ValueError(
                        f"Program {program.name} with size :{program.size} bytes cannot be allocated in the {program.memory}\
    ...
    def mapUnmappedData(self):
        for core in self.cores:
            if core.unmappedDataSize:
                lowAddress, highAddress = self.allocateMemory(
                    core.unmappedDataMemory, core.unmappedDataSize
                )
                if not lowAddress or not highAddress:
                    raise ValueError(
                        f"Unmapped data for core {core.name} with size :{core.unmappedDataSize} bytes cannot be allocated in the {core.unmappedDataMemory}\
    ...
    def mapUserCode(self):
        for core in self.cores:
            if core.userCodeSize:
                lowAddress, highAddress = self.allocateMemory(
                    core.userCodeMemory, core.userCodeSize
                )
                if not lowAddress or not highAddress:
                    raise ValueError(
                        f"User code for core {core.name} with size :{core.userCodeSize} bytes cannot be allocated in the {core.userCodeMemory}\
    ...
    def mapStaticData(self):
        for core in self.cores:
            if core.staticDataSize:
                lowAddress, highAddress = self.allocateMemory(
                    core.staticDataMemory, core.staticDataSize
                )
                if not lowAddress or not highAddress:
                    raise ValueError(
                        f"Unmapped code for core {core.name} with size :{core.staticDataSize} bytes cannot be allocated in the {core.staticDataMemory}\
        ...
        for chunk in memory.freeChunks:
            if size == chunk.size and not (chunk.lowAddress % size):
                ...
                if not (chunk.lowAddress % size):
                    ...
                    memory.freeChunks.append(
                        Chunk(highAddress, chunk.highAddress, chunkNewSize)
                    )
                    memory.freeChunks.remove(chunk)
                    break
                else:
                    ...
                    if size == (chunk.highAddress - newLowAddress):
                        ...
                        memory.freeChunks.append(
                            Chunk(chunk.lowAddress, newLowAddress, chunkNewSize)
                        )
                        memory.freeChunks.remove(chunk)
                        break
