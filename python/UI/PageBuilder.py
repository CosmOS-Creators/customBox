...
class cardPage(CardWidget):
	def __init__(self, parent: QWidget, page: QWidget, name: str, mapped_element: ConfigElement, uiPage: UiPage):
...
class cardedPage(QScrollArea, ContainerPage):
	...
	def create_element_handler(self, subconfig: Subconfig, uiPage: UiPage):
		element_name, result = QInputDialog.getText(self, "Element creation", "What name should the new element have?", QLineEdit.Normal)
		if(result):
			try:
			...
			new_page = cardPage(self.page_widget, card, element_name, new_element, uiPage)
			new_page.addHandlers(lambda: self.create_element_handler(subconfig, uiPage), self.delete_handler)
...
class tabbedPage(QTabWidget, ContainerPage):
	...
	def add_handler(self, subconfigs: Dict[str, Subconfig]):
		if(len(subconfigs) == 1):
			...
			subconfig_key, result = QInputDialog.getItem(self, "Element creation", "Choose the subconfig for which to create an element for:", subconfig_keys, 0)
			if(result):
	...
	def __init__(self, parent: QWidget, uiPage: UiPage):
		...
		if(uiPage.allowElementCreation):
			add_button = iconButton(self, "add", "Add a new element", clicked=lambda: self.add_handler(subconfigs))
