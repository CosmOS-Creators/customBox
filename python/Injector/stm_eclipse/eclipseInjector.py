...
LINE = "----------------------------------------------------------------------\n"
flagLineTemplate = (
    '									<listOptionValue builtIn="false" value="{{flag}}"/>{{"\n"}}'
)
defineLineTemplate = (
...
includeLineTemplate = (
    '									<listOptionValue builtIn="false" value="{{pathToUnit}}/inc"/>{{"\n"}}'
)
...
def injectConfig(workspace, compilerCfg):
    ...
    if len(compilerCfg.cores) != len(workspace.cprojectPaths):
        raise ValueError(
            "Number of cores in compilerCfg.json is not equal to the cprojectPaths in workspace.json"
        )
    ...
    linkedResourcesFoundTemplateInstance = Template(
        linkedResourcesMultilineFoundTemplate
    )
    ...
    for cprojectPath in workspace.cprojectPaths:
        ...
        if startFlagsSection:
            raise ValueError(
                "The flags section in the .cproject file was not found, \
        ...
        for line in inputFile:
            if startIncludesSection and re.search(regexEndOfSection, line):
                for unit in workspace.CosmOSCoreModules:
                    newDefLine = incTemplateInstance.render(
                        pathToUnit="{}/{}".format(workspace.CosmOSCorePath, unit)
                    )
                    writeFile.write(newDefLine)
                for unit in workspace.CosmOSGeneratedCoreModules:
                    newDefLine = incTemplateInstance.render(
                        pathToUnit="{}/{}".format(
                            workspace.CosmOSGeneratedCorePath, unit
                        )
                    )
                    writeFile.write(newDefLine)
                newDefLine = incTemplateInstance.render(
                    pathToUnit=workspace.CosmOSIntegrationLayerPath
                )
                writeFile.write(newDefLine)
                newDefLine = incTemplateInstance.render(
                    pathToUnit=workspace.CosmOSApplicationLayerPath
                )
                writeFile.write(newDefLine)
                writeFile.write(line)
                startIncludesSection = False
                ...
                matchDefine = False
                for unit, generatedUnit in zip_longest(
                    workspace.CosmOSCoreModules, workspace.CosmOSGeneratedCoreModules
                ):
                    if (
                        (
                            re.search(
                                "{}/{}".format(workspace.CosmOSCorePath, unit), line
                            )
                        )
                        or (re.search(workspace.CosmOSIntegrationLayerPath, line))
                        or (re.search(workspace.CosmOSApplicationLayerPath, line))
                        or (
                            re.search(
                                "{}/{}".format(
                                    workspace.CosmOSGeneratedCorePath, generatedUnit
                                ),
                                line,
                            )
                        )
        ...
        for line in inputFile:
            ...
            if startSourcesSection and re.search(regexEndOfSource, line):
                newDefLine = sourceTemplateInstance.render(
                    sourcePath=workspace.CosmOSIntegrationLayerPath
                )
                writeFile.write(newDefLine)
                newDefLine = sourceTemplateInstance.render(
                    sourcePath=workspace.CosmOSApplicationLayerPath
                )
                writeFile.write(newDefLine)
                newDefLine = sourceTemplateInstance.render(
                    sourcePath=workspace.CosmOSCorePath
                )
                writeFile.write(newDefLine)
                newDefLine = sourceTemplateInstance.render(
                    sourcePath=workspace.CosmOSGeneratedCorePath
                )
                writeFile.write(newDefLine)
                writeFile.write(line)
                startSourcesSection = False
                ...
                matchDefine = False
                if (
                    (re.search(workspace.CosmOSCorePath, line))
                    or (re.search(workspace.CosmOSGeneratedCorePath, line))
                    or (re.search(workspace.CosmOSApplicationLayerPath, line))
                    or (re.search(workspace.CosmOSIntegrationLayerPath, line))
                ):
                    matchDefine = True
    ...
    if len(compilerCfg.cores) > 1:  # multicore project
        for projectPath in workspace.projectPaths:
            ...
            print(
                "{}{}{}{}".format(
                    LINE, "-> Injecting ", projectPath, " linked resources"
                )
            )
            # inject linkedResources
            for line in inputFile:

                if startLinkedResourcesSection and re.search(
                    regexEndOfLinkedResources, line
                ):
                    startLinkedResourcesSection = False
                    if not (corePathIsLinked):
                        newDefLine = linkedResourcesTemplateInstance.render(
                            sourcePath=workspace.CosmOSCorePath
                        )
                        writeFile.write(newDefLine)
                    ...
                    if (
                        matchNameGeneratedCorePath == matchUriGeneratedCorePath
                        and matchNameGeneratedCorePath
                    ):
                    ...
                    if (
                        startUriSection
                        and matchNameCorePath != matchUriCorePath
                        and not (corePathIsLinked)
                    ):
                        newDefLine = linkedResourcesFoundTemplateInstance.render(
                            sourcePath=workspace.CosmOSCorePath
                        )
                        writeFile.write(newDefLine)
                        startNameSection = False
                    ...
                    if (
                        startUriSection
                        and matchNameGeneratedCorePath != matchUriGeneratedCorePath
                        and not (generatedCorePathIsLinked)
                    ):
                        newDefLine = linkedResourcesFoundTemplateInstance.render(
                            sourcePath=workspace.CosmOSGeneratedCorePath
                        )
                        ...
                        startUriSection
                        and matchNameAL != matchUriAL
                        and not (ALPathIsLinked)
                    ):
                        newDefLine = linkedResourcesFoundTemplateInstance.render(
                            sourcePath=workspace.CosmOSApplicationLayerPath
                        )
                        ...
                        startUriSection
                        and matchNameCIL != matchUriCIL
                        and not (CILPathIsLinked)
                    ):
                        newDefLine = linkedResourcesFoundTemplateInstance.render(
                            sourcePath=workspace.CosmOSIntegrationLayerPath
                        )
...
def main():
    ...
    print(
        "{}{}{}".format(
            LINE,
            "-> STM CUBE IDE cproject and project files were successfully injected!\n",
            LINE,
        )
    )
