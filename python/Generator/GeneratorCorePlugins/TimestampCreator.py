...
class timeStampPlugin(PluginSkeleton.GeneratorPlugin):
    def __init__(
        self,
        disableTimestamps: bool = False,
        timestampFormat: str = None,
        timestampIgnoreRegex: str = None,
    ):
        ...
        if timestampFormat:
            ...
            if not timestampIgnoreRegex:
                raise AttributeError(
                    f'Error for timestamp plugin init. Parameter "timestampFormat" is given but parameter "timestampIgnoreRegex" is not. but it is mandatory.'
                )
            else:
    ...
    def preFileGeneration(
        self, currentTemplateDict: dict, systemConfig, file_path: Path
    ):
        if self.__disable_timestamps:
            currentTemplateDict[templateDateVariableName] = ""
        else:
            currentTemplateDict[templateDateVariableName] = datetime.now().strftime(
                self.__time_format
            )
