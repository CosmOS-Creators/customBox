	...
	def __ensure_hex_consistency(self, value):
		if(isinstance(value, str)):
			if(not value.upper().startswith("0X" )):
				raise ValueError(f'Error parsing "{self.globalID}", the 0x prefix should always be used for hex values in the config files. If you want to specify a decimal value instead please define the value as an integer.')
	...
	def checkValue(self, valueInput: Union[str, int]):
		...
		if(self.alignment is not None):
			if(not helpers.check_alignment(convertedInput, self.alignment)):
				reportValidationError(f'The input value ({valueInput}) is not a power of ({self.alignment}) but that is specified as required')
