...
class TestClassLinkFunctions:
    ...
    def mocked_basic_config_structure(self):
        ...
        boolType = AttributeTypes.BoolType(
            {"label": "Bool type", "type": "bool"}, "config/:attribute"
        )
        attribLookup = dict()
    ...
    def test_constructor(self):
        ...
        with patch.object(
            Link,
            "split",
            return_value=("mockedConfig", "MockedElement", "MockedAttribute"),
        ) as mocked_split_method:
        ...
        mocked_split_method.assert_called_once_with(
            "config/element:attribute", Link.EMPHASIZE_ELEMENT
        )
        with patch.object(
            Link, "split", return_value=("mockedConfig", None, None)
        ) as mocked_split_method:
    ...
    def test_link_hasAnyParts_method(self):
        ...
        link = Link()
        expectations = [True, False, False, False, False, False, False]
    ...
    def test_force_method(
        self, mocked_basic_config_structure: ConfigTypes.Configuration
    ):
    ...
    def test_resolve_methods(
        ...
        assert noElementLink.resolve(mocked_basic_config_structure) == [
            (attribute, element)
        ]
        with pytest.raises(ValueError):
