...
class TestClassLinkFunctions:
	...
	def test_defaultValue_methods(self):
		defaults = [
			("", 	AttributeTypes.StringType({"type": "string", 					"label": "test"}, "cores/:name1")),
			(False, AttributeTypes.BoolType({"type": "bool", 						"label": "test"}, "cores/:name2")),
			(0, 	AttributeTypes.IntType({"type": "int", 							"label": "test"}, "cores/:name3")),
			(0, 	AttributeTypes.FloatType({"type": "float", 						"label": "test"}, "cores/:name4")),
			([], 	AttributeTypes.ReferenceListType({"type": "referenceList", 		"label": "test"}, "cores/:name5")),
			([], 	AttributeTypes.StringListType({"type": "stringList", 			"label": "test"}, "cores/:name6")),
			(None, 	AttributeTypes.SelectionType({"type": "selection", 				"label": "test", "elements": []}, "cores/:name10")),
			(0, 	AttributeTypes.HexType({"type": "hex", 							"label": "test"}, "cores/:name7")),
			(0, 	AttributeTypes.SliderType({"type": "slider", 					"label": "test"}, "cores/:name8")),
			(None, 	AttributeTypes.ParentReferenceType({"type": "parentReference", 	"label": "test"}, "cores/:name9"))
		]
	...
	def test_parseAttribute_function(self):
		...
		for typeStr, classType in resultingTypes.items():
			assert type(AttributeTypes.parseAttribute({"type": typeStr, "label": "test"}, "cores/:name")) is classType
		# selection needs some special keys
		assert type(AttributeTypes.parseAttribute({"type": "selection", "label": "test", "elements": []}, "cores/:name")) is AttributeTypes.SelectionType

	def test_attributeTypes_baseClass(self, mocked_basic_cores_config_structure: ConfigTypes.Configuration):
		...
		missconfiguredType.link(mocked_basic_cores_config_structure, attribInst) # does nothing

	def test_parentReference_Link_method(self, mocked_cores_program_config_structure: ConfigTypes.Configuration):
		...
		assert len(expected_items) == len(core_0_attributes_list)
		parent_linked_programs = core_0.getReferenceObject("programs")
		assert parent_linked_programs.references == {"program_0": program_0, "program_1": program_1}

	def test_parentReference_init_method(self):
		parentReference = AttributeTypes.ParentReferenceType({"type": "parentReference", "label": "test", "tooltip": "test info"}, "programs/:core")
		assert parentReference.globalID == "programs/:core"
		assert parentReference.needsLinking == True
		with pytest.raises(KeyError):
			AttributeTypes.ParentReferenceType({"type": "parentReference", "hidden": True}, "programs/:core")
		with pytest.raises(KeyError):
			AttributeTypes.ParentReferenceType({"type": "parentReference", "placeholder": True}, "programs/:core")
		with pytest.raises(KeyError):
			AttributeTypes.ParentReferenceType({"type": "parentReference", "min": 0}, "programs/:core")
		with pytest.raises(KeyError):
			AttributeTypes.ParentReferenceType({"type": "parentReference", "max": 0}, "programs/:core")
		with pytest.raises(KeyError):
			AttributeTypes.ParentReferenceType({"type": "parentReference", "step": 5}, "programs/:core")
