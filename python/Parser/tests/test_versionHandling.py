...
class TestCompatabilityManager:
    ...
    @mock.patch.object(
        CompatabilityManager, "get_current_version", return_value=Version("1.1.0")
    )
    ...
    @mock.patch.object(
        CompatabilityManager, "get_current_version", return_value=Version("2.0.0")
    )
    def test_indirect_upgrade_path(
        self, get_current_version, upgrade_lookup_test: Dict
    ):
        CompatabilityManager.UPGRADE_LOOKUP = upgrade_lookup_test
        test_data = {const.VERSION_KEY: "1.0.0"}
        upgraded_data = CompatabilityManager.upgrade(test_data)
