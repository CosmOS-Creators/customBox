...
class Version:
    ...
    def matches(self, version: Union[str, Version]):
        ...
        if (
            compare_version.major == self.major
            and compare_version.minor == self.minor
            and compare_version.patch == self.patch
        ):
            return True
        else:
            return False
...
class CompatabilityManager:
    ...
    def _find_upgrade_path(
        from_version: Version, to_version: Version, upgrade_path: List = []
    ) -> List[upgradeHandlers.baseUpgradeHandler]:
        if from_version in CompatabilityManager.UPGRADE_LOOKUP:
            upgraded_version, handler = CompatabilityManager.UPGRADE_LOOKUP[
                from_version
            ]
            ...
            if upgraded_version == to_version:
                ...
                CompatabilityManager._find_upgrade_path(
                    upgraded_version, to_version, upgrade_path
                )
                return upgrade_path
        else:
            raise Exception(
                f"Could not find a upgrade path from version {from_version} to version {to_version}"
            )
    ...
    def upgrade(input: Dict):
        ...
        if current_file_version > CompatabilityManager.get_current_version():
            raise Exception(
                f"The file format is from a newer parser version. Downgrading is not possible, please update your parser to at least version {str(current_file_version)} to open this file."
            )
        elif current_file_version == CompatabilityManager.get_current_version():
            return input
        upgrade_path = CompatabilityManager._find_upgrade_path(
            current_file_version, CompatabilityManager.get_current_version()
        )
        upgraded_data = input
