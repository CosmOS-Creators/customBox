...
AttributeCollectionType = NewType(
    "AttributeCollectionType", Dict[str, AttributeTypes.AttributeType]
)
...
def processAttributes(config: Dict[str, object]) -> AttributeCollectionType:
        ...
        for attribute in config[configName][const.ATTRIBUTES_KEY]:
            ...
            if const.INHERIT_KEY in currentAttribute:
                if not Link.isGlobal(currentAttribute[const.INHERIT_KEY]):
                    currentAttribute[const.INHERIT_KEY] = Link.construct(
                        config=configName, attribute=currentAttribute[const.INHERIT_KEY]
                    ).getLink()
                AttributesToInherit[globalIdentifier.getLink()] = currentAttribute
            else:
                try:
                    attributeCollection[
                        globalIdentifier.getLink()
                    ] = AttributeTypes.parseAttribute(
                        currentAttribute, globalIdentifier
                    )
                except KeyError as e:
                    raise KeyError(
                        f'Invalid attribute in config "{configName}" for attribute "{attribute}": {e}'
                    ) from e
    for attribLink in AttributesToInherit:
        ...
        try:
            ...
            raise KeyError(
                f'In config "{link.config}" the attribute inherit target "{attrib[const.INHERIT_KEY]}" does not match any known attributes'
            )
        if baseAttribute.is_inherited:
            raise Exception(
                f'In config "{link.config}" it was tried to inherit from "{attrib[const.INHERIT_KEY]}" but this attribute is already inherited and inheritance nesting is not supported at the moment.'
            )
        attributeCollection[attribLink] = baseAttribute.create_inheritor(
            attrib, attribLink
        )
...
def discoverConfigFiles(configPath: Union[str, List[str]]) -> List[Path]:
    ...
    for currentConfigPath in configPaths:
        if not os.path.exists(currentConfigPath) or not os.path.isdir(
            currentConfigPath
        ):
...
class ConfigParser:
        ...
        if isinstance(workspace, list):
            ...
            for config in workspace:
                if isinstance(config, str):
                    ...
                    raise TypeError(
                        f"Only supported types as a config input are List[str | Path] but got List[{type(config)}] instead."
                    )
        else:
            raise TypeError(
    ...
    def parse(self) -> ConfigTypes.Configuration:
        if self.__workspace is not None:
            ...
            raise Exception(
                f"Configuration was not passed to the ConfigParser correctly. Cannot proceed."
            )
        jsonConfigs = dict()
        ...
        for configFile in configFiles:
            with configFile.open("r") as currentFile:
                ...
                if not const.configFileNameRegex.match(configCleanName):
                    raise Exception(
                        f'Config file names are oly allowed to contain lower case alphanumeric characters but the file "{configFile}" would generate a config named "{configCleanName}" which would violate this restriction'
                    )
                if configCleanName in jsonConfigs:
                    raise Exception(
                        f'Config file names have to be unique but the files "{configFileNames[configCleanName]}" and "{configFile}" have the same name({configCleanName}) thus are considered duplicated.'
                    )
                try:
                    loaded_json_config = json.load(currentFile)
                except json.JSONDecodeError as e:
                    raise Exception(
                        f'Config file "{configFile}" is not a valid json: {str(e)}'
                    ) from e
                try:
                    config_file_sanity_check(loaded_json_config)
                except KeyError as e:
                    raise KeyError(
                        f'Error in config file "{configFile}": {str(e)}'
                    ) from e
        ...
        for config_name, config in jsonConfigs.items():
            configuration.createSubconfigFromDefinition(
                config_name, config, configFileNames[config_name]
            )
